                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow') :
                            ...
                            let wel = await fetch(`https://oni-chan.my.id/api/canvas/welcome_v1?ppurl=${pp}&bgurl=https://telegra.ph/file/7f827ca45c833542777f0.jpg&username=${await this.getName(user)}&totalmember=${groupMetadata.participants.length}&secondtext=wah%20member%20baru%20&apikey=`)
...
mods: 'Printah ini hanya di gunakan oleh *Moderator BOT* !'}[type]
  if (mod) return conn.reply(m.chat, ucapan() + tag + '\n' + mod, fkontak)
let prm = {
